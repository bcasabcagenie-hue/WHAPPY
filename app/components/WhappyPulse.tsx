...
export function WhappyPulse({ open, onClose, onNavigate, unread, orderCount, cartCount, groupCount, syncStatus, userName, demo }: { open: boolean; onClose: () => void; onNavigate: (space: PulseSpace) => void; unread: number; orderCount: number; cartCount: number; groupCount: number; syncStatus: "local" | "syncing" | "synced" | "offline"; userName: string; demo: boolean }) {
  ...
  return <div className="pulse-layer" role="dialog" aria-modal="true" aria-label="Centre de commandes Whappy">
    ...
    <section className="pulse-center">
      <header><div className="pulse-brand"><span>W</span><div><small>WHAPPY PULSE</small><strong>Bonjour {userName.split(" ")[0] || "vous"}</strong></div></div><div className={`pulse-system ${syncStatus}`}><i />{syncLabel}</div><button onClick={onClose} aria-label="Fermer">×</button></header>
