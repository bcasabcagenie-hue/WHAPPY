...
export function ContactsSpace({ search, cloud, userId, userName, cloudGroups, onCreateGroup, onMessage, onProfile, onCall, onUpdateGroup, notify }: {
  search: string;
  cloud: boolean;
  userId: string;
  userName: string;
  ...
  onProfile: (contact: { name: string; phone?: string; mark: string; note: string; online: boolean }) => void;
  ...
  return <div className="contact-space">
    <section className="contact-summary"><div><span>CARNET WAPI</span><h2>Les bonnes personnes,<br/><em>au bon moment.</em></h2><p>Contacts, groupes et professionnels réunis autour de votre numéro.</p></div><div className="contact-stats"><span><b>{contactList.length}</b> contact{contactList.length === 1 ? "" : "s"}</span><span><b>{cloudGroups.length}</b> groupe{cloudGroups.length === 1 ? "" : "s"}</span><span><b>0</b> pro vérifié</span></div></section>
...
const services: Array<{ icon: string; title: string; note: string; destination?: Destination; accent?: boolean }> = [
  ...
  { icon: "◎", title: "Mon Jumeau numérique", note: "Votre vendeur vidéo autorisé", destination: "twin", accent: true },
...
export function SuperHub({ go, orderCount, onOrders, notify, search = "" }: { go: (destination: Destination) => void; orderCount: number; onOrders: () => void; notify: (text: string) => void; search?: string }) {
  ...
  useEffect(() => {
    ...
    void QRCode.toDataURL(payload, { width: 280, margin: 1, color: { dark: "#0066CF", light: "#FFFFFF" } }).then(setQrUrl).catch(() => setQrUrl(""));
  ...
  return <div className="super-hub">
    ...
    <section className="hub-body">
      ...
      <header className="hub-section-title"><div><small>MINI-SERVICES WAPI</small><h3>{query ? `Résultats pour « ${search} »` : "Tout ce dont vous avez besoin"}</h3></div></header>
