...
import { clearWhappyAccountCache, readWhappyCache, writeWhappyCache } from "@/lib/whappy-local-cache";
...
const ChannelsSpace = dynamic(() => import("@/app/components/ChannelsSpace").then((m) => m.ChannelsSpace), { ssr: false });
...
const ANDROID_APP = {
  url: "https://raw.githubusercontent.com/bcasabcagenie-hue/WHAPPY/codex/sites-deploy-lite/public/downloads/WAPI-Android-1.9.5-native.apk",
...
const messages = [
  { name: "Amina M.", text: "Le troc est accepté pour le canapé ?", time: "Maintenant", mark: "AM", color: "#0066CF", unread: 2, mood: "Cherche une belle pièce pour son salon", badge: "ACHETEUSE FIABLE", streak: 12, verified: true, accountType: "personal" as const },
  { name: "Junior K.", text: "Je peux livrer le MacBook cet après-midi.", time: "12:08", mark: "JK", color: "#0066CF", unread: 1, mood: "Disponible pour une livraison rapide", badge: "VENDEUR VÉRIFIÉ", streak: 28, verified: true, accountType: "personal" as const },
  { name: "Mokabi Store", text: "Votre commande est prête ✦", time: "11:42", mark: "MS", color: "#0066CF", unread: 0, mood: "Collection N'Tela en direct ce soir", badge: "BOUTIQUE PRO", streak: 54, verified: true, accountType: "business" as const },
  { name: "Design Crew", text: "Nadia : rendez-vous confirmé demain", time: "Hier", mark: "DC", color: "#0066CF", unread: 0, mood: "Créateurs disponibles cette semaine", badge: "GROUPE ACTIF", streak: 19, verified: true, accountType: "community" as const },
  { name: "Maison Noki", text: "Canapé disponible pour échange ou vente.", time: "10:26", mark: "MN", color: "#0066CF", unread: 0, mood: "Maison et décoration à Bacongo", badge: "VENDEUR VÉRIFIÉ", streak: 17, verified: true, accountType: "business" as const },
  { name: "Nadia M.", text: "Studio photo disponible cette semaine.", time: "09:14", mark: "NM", color: "#0066CF", unread: 0, mood: "Service local · Moungali", badge: "PROFESSIONNELLE VÉRIFIÉE", streak: 21, verified: true, accountType: "personal" as const },
];
...
export default function Home() {
  ...
  const [radioTab, setRadioTab] = useState<"live" | "podcasts">("live");
  ...
  function go(next: Space) {
    if (next === "radio") setRadioTab("live");
    else if (next === "podcasts") setRadioTab("podcasts");
    setSpace(next);
  ...
  const searchPlaceholders: Record<Space,string> = {
    ...
    channels: "Rechercher une chaîne, un média ou un créateur…",
    rooms: "Rechercher une salle, une communauté ou un thème…",
    radio: "Rechercher un podcast, une émission ou un auditeur…",
    podcasts: "Rechercher un podcast ou un épisode…",
    orbit: "Rechercher dans les Actus…",
  ...
  return <main className="nova-shell whappy-blue">
    <aside className="nova-rail">
      ...
      <nav aria-label="Espaces Wapi App">
        ...
        <Rail active={space === "channels"} icon="▤" label="Chaînes" onClick={() => go("channels")} />
      ...
      <div className="rail-tools">
        ...
        <button className={space === "twin" ? "active" : ""} onClick={() => go("twin")}><span>◎</span><small>Mon Jumeau numérique</small></button>
    ...
    <section className="nova-stage">
      <header className="nova-topbar">
        <div className="topbar-identity">
          <button className="mobile-logo" onClick={() => go("inbox")} aria-label="Messages Wapi">
            <Image src="/wapi-logo.svg" alt="Logo officiel Wapi" width={40} height={40} priority />
          </button>
      ...
      {space === "contacts" && <ContactsSpace search={search} cloud={Boolean(userId)} userId={userId} userName={auth.currentUser?.displayName||profileName||"Vous"} cloudGroups={groups} onCreateGroup={createTrackedGroup} onUpdateGroup={updateTrackedGroup} notify={notify} onProfile={setSelectedContactProfile} onCall={(contact)=>{if (contact.includes("appel de groupe")) notify("Appel de groupe : le transport média SFU WAPI doit être activé avant de connecter plusieurs appareils."); else setCall({contact,video:false})}} onMessage={(contact)=>{setDirectPeer(null);setDirectPhone(contact.phone||"");setDirectCompose((value)=>value+1);go("inbox");notify(contact.phone?`Ouverture de la conversation avec ${contact.name}`:`Entrez le numéro Wapi de ${contact.name}`)}} />}
      {space === "channels" && <ChannelsSpace userId={userId||"local-preview"} userName={accountName} search={search} cloud={Boolean(userId)} notify={notify} onOpenGroupSpaces={() => go("rooms")} />}
      {space === "rooms" && <RoomsSpace userId={userId||"demo-user"} userName={accountName} search={search} cloud={Boolean(userId)} notify={notify} />}
      <RadioStudio
        active={space === "radio" || space === "podcasts"}
        tab={radioTab}
        onTabChange={(nextTab) => go(nextTab === "podcasts" ? "podcasts" : "radio")}
    ...
    {profileOpen && <ProfilePanel name={accountName} phone={auth.currentUser?.phoneNumber || `${countryCode} ${phone || "06 000 00 00"}`} accountId={userId || "local-preview"} syncStatus={syncStatus} photoUrl={profilePhotoUrl} founder={founderProfile} onSaveProfile={saveProfileChanges} notify={notify} onClose={() => setProfileOpen(false)} go={(destination) => { setProfileOpen(false); go(destination); }} onOpenShop={() => { setProfileOpen(false); setShopOpen(true); }} onOpenOrders={() => { setProfileOpen(false); setOrdersOpen(true); }} onSignOut={async () => { if (userId) clearWhappyAccountCache(userId); if (auth.currentUser) await signOut(auth); setProfileOpen(false); setAuthenticated(false); }} />}
    {selectedContactProfile && <ContactProfilePanel contact={selectedContactProfile} onClose={() => setSelectedContactProfile(null)} onMessage={() => { const contact = selectedContactProfile; setSelectedContactProfile(null); setDirectPeer(null); setDirectPhone(contact.phone || ""); setDirectCompose((value) => value + 1); go("inbox"); notify(`Conversation avec ${contact.name} ouverte`); }} onCall={() => { const contact = selectedContactProfile; setSelectedContactProfile(null); setCall({ contact: contact.name, video: false }); }} />}
...
function ReconnectScreen() {
  return <main className="whappy-reconnect" aria-live="polite">
    <div className="whappy-reconnect-card">
      <Image className="whappy-reconnect-logo" src="/wapi-logo.svg" alt="Logo Wapi" width={64} height={64} priority />
...
function UpdatesSpace({ go, story }: { go: (space: Space) => void; story: ReactNode }) {
  const sections: Array<{ title:string; note:string; items:Array<{icon:string;label:string;detail:string;space:Space}> }> = [
    { title:"Chaînes et audio", note:"Vos programmes et créateurs au même endroit", items:[
      {icon:"▤",label:"Chaînes",detail:"Médias et créateurs",space:"channels"},
      {icon:"◉",label:"Radios",detail:"Émissions en direct",space:"radio"},
      {icon:"◖",label:"Podcasts",detail:"Écouter ou reprendre",space:"podcasts"},
    ]},
    { title:"Pour vous", note:"Divertissement et découvertes WAPI", items:[
      {icon:"●",label:"En direct",detail:"Lives en cours",space:"live"},
      {icon:"♞",label:"Jeux",detail:"Jouer et progresser",space:"games"},
      {icon:"◇",label:"Découvrir",detail:"Marché et services",space:"market"},
    ]},
  ];
  return <div className="updates-space"><header><div><small>WAPI ACTUS</small><h2>Vos Stories.<br/><em>Vos programmes.</em></h2><p>Les Stories restent personnelles et temporaires. Les chaînes, radios, podcasts et jeux vivent séparément en dessous.</p></div><span>24 H</span></header><section className="updates-stories">{story}</section>{sections.map((section)=><section className="updates-section" key={section.title}><header><div><h3>{section.title}</h3><p>{section.note}</p></div></header><div>{section.items.map((item)=><button key={item.label} onClick={()=>go(item.space)}><span>{item.icon}</span><div><strong>{item.label}</strong><small>{item.detail}</small></div><b>→</b></button>)}</div></section>)}</div>;
}

export function Orbit({ go, setModal, setLiveIndex, notify, saved, setSaved, ad, onAdClick, footer, userName, founder }: { go: (space: Space) => void; setModal: (modal: "sell" | "seek" | "live") => void; setLiveIndex: (index: number) => void; notify: (text: string) => void; saved: Record<string, boolean>; setSaved: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; ad?: AdCampaign; onAdClick: (campaign: AdCampaign) => void; footer?: React.ReactNode; userName: string; founder?: boolean }) {
...
function LiveSpace({ setModal, setLiveIndex }: { setModal: (type: "live") => void; setLiveIndex: (index: number) => void }) {
  ...
  return <div className="space-scroll live-space">
    <section className="live-command twin-live-command"><div className="live-command-copy"><span className="signal"><i/> WAPI LIVE SHIFT™</span><h2>Vous commencez.<br/><em>Votre Jumeau numérique continue.</em></h2><p>Lancez un direct avec votre caméra, avec votre Jumeau numérique vidéo autorisé, ou passez de l&apos;un à l&apos;autre sans couper le live.</p><div className="live-mode-switch"><button className={mode==="human"?"active":""} onClick={()=>setMode("human")}><span>▣</span><strong>Moi, en direct</strong><small>Caméra et voix réelles</small></button><button className={mode==="twin"?"active":""} onClick={()=>setMode("twin")}><span>◎</span><strong>Mon Jumeau numérique</strong><small>Vidéo IA autorisée</small></button><button className={mode==="relay"?"active":""} onClick={()=>setMode("relay")}><span>⇄</span><strong>Passage de relais</strong><small>Humain + Jumeau</small></button></div><button onClick={() => setModal("live")}>● Entrer dans le studio</button><small className="live-safety">✓ Votre image uniquement · Jumeau signalé comme IA · Arrêt immédiat à tout moment</small></div><div className={`studio-preview shift-preview ${mode}`}><span className="preview-live">● LIVE · 00:12:48</span><div className="shift-status"><span>{mode==="human"?"CAMÉRA RÉELLE":mode==="twin"?"JUMEAU NUMÉRIQUE IA ACTIF":"PASSAGE DE RELAIS"}</span><b>{mode==="relay"?"Transition dans 00:08":"Contrôle créateur actif"}</b></div><div className="shift-stage"><div className={`shift-person human-feed ${mode==="human"||mode==="relay"?"on":""}`}><span>CB</span><small>VOUS · LIVE</small></div><div className="shift-link">{mode==="relay"?"⇄":"＋"}</div><div className={`shift-person twin-feed ${mode==="twin"||mode==="relay"?"on":""}`}><span>CB</span><small>JUMEAU · IA</small><b>IA</b></div></div><div className="preview-comments"><span>Ça existe en bleu ?</span><span>Le Jumeau numérique parle aussi lingala ?</span></div><div className="preview-product"><i>◇</i><span><small>PRODUIT ÉPINGLÉ</small><strong>Montre Kongo One</strong><b>42 000 FCFA</b></span><button>Acheter</button></div></div></section>
    <section className="space-content"><div className="shift-explainer"><div><span>01</span><strong>Vous créez la confiance</strong><p>Présentez, racontez et répondez vous-même.</p></div><i>→</i><div><span>02</span><strong>Le Jumeau numérique prend le relais</strong><p>Il reprend seulement le script et les produits approuvés.</p></div><i>→</i><div><span>03</span><strong>Vous revenez instantanément</strong><p>Un geste suffit pour reprendre la caméra en direct.</p></div></div><SectionTitle overline="EN CE MOMENT" title="Des directs augmentés, pas automatisés" action="Créer mon direct" onClick={() => setModal("live")}/><div className="big-live-grid">{lives.map((live,index)=><button key={live.host} className={`big-live ${live.tone} has-stream`} style={{backgroundImage:`linear-gradient(180deg,rgba(3,8,11,.04) 20%,rgba(3,8,11,.84) 100%),url(${live.poster})`}} onClick={() => setLiveIndex(index)}><span><i/> {live.badge}</span><div className="host-face">{live.host.split(" ").map(x=>x[0]).join("").slice(0,2)}</div><div><small>{live.host} · {live.viewers} spectateurs</small><h3>{live.title}</h3><p>Produit épinglé : {live.product}</p><b>{live.price} FCFA</b><em>▶ Ouvrir le direct</em></div></button>)}</div><div className="live-features"><div><span>⇄</span><strong>Live Shift™</strong><p>Passez de votre caméra au Jumeau sans interrompre l&apos;audience.</p></div><div><span>◌</span><strong>Questions filtrées</strong><p>Le Jumeau numérique répond uniquement dans les limites que vous avez approuvées.</p></div><div><span>⏱</span><strong>Vente continue</strong><p>Quittez la scène ; votre boutique reste présentée sous votre contrôle.</p></div></div></section>
  </div>;
}
...
function TwinSpace({ step, setStep, consent, setConsent, notify }: { step:number; setStep:(n:number)=>void; consent:boolean; setConsent:(v:boolean)=>void; notify:(text:string)=>void }) {
  ...
  function saveDraft(){try{localStorage.setItem("whappy-twin-draft",JSON.stringify({product,price,tone,languages,updatedAt:new Date().toISOString()}));notify("Jumeau numérique enregistré en brouillon sur cet appareil");}catch{notify("Le stockage local est bloqué par votre navigateur");}}
  return <div className="space-scroll twin-space"><section className="twin-hero"><div className="twin-copy"><span className="signal"><i/> STUDIO JUMEAU NUMÉRIQUE · VOTRE IMAGE, VOTRE CONTRÔLE</span><h2>Vous créez une fois.<br/><em>Votre Jumeau numérique continue de vendre.</em></h2><p>Enregistrez votre propre vidéo. Wapi prépare un présentateur numérique pour les produits, langues et formats que vous autorisez.</p><div className="safety-chips"><span>✓ Consentement explicite</span><span>✓ Révocable à tout moment</span><span>✓ Marqué comme IA</span></div></div><div className="twin-visual"><div className="scan-lines"/><div className="human"><span>CB</span><small>VOUS</small></div><div className="transfer">··· ✦ ···</div><div className="digital"><span>CB</span><small>JUMEAU NUMÉRIQUE IA</small><b>IA</b></div></div></section><section className="twin-builder"><div className="builder-steps">{["Consentement","Enregistrement","Produits","Personnalité","Publication"].map((x,index)=><button className={step===index+1?"active":step>index+1?"done":""} onClick={()=>{if(index===0||consent)setStep(index+1);else notify("Validez d’abord votre consentement")}} key={x}><span>{step>index+1?"✓":index+1}</span><small>{x}</small></button>)}</div><div className="builder-card">{step===1&&<><span className="builder-icon">◎</span><h3>Votre identité vous appartient</h3><p>Wapi utilise uniquement les vidéos de vous-même que vous fournissez. Votre Jumeau numérique ne peut pas représenter une autre personne et chaque vidéo générée porte le label « Créé avec un Jumeau numérique IA ».</p><label className="consent"><input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)}/><span>Je confirme créer un Jumeau numérique à partir de ma propre image et j&apos;accepte son utilisation uniquement pour mes contenus Wapi autorisés.</span></label><button disabled={!consent} onClick={()=>setStep(2)}>Continuer vers l&apos;enregistrement ↗</button></>}{step===2&&<TwinRecorder onComplete={()=>{notify("Capsule vidéo validée");setStep(3)}}/>}{step===3&&<><span className="builder-icon">◇</span><h3>Ajoutez ce que votre Jumeau numérique vendra</h3><p>Définissez le produit et le prix que le Jumeau numérique pourra présenter. Vous pourrez toujours les modifier.</p><div className="twin-product-fields"><label>Produit<input value={product} onChange={e=>setProduct(e.target.value)} placeholder="Nom du produit"/></label><label>Prix ou offre<input value={price} onChange={e=>setPrice(e.target.value)} placeholder="Prix en FCFA"/></label></div><button disabled={!product.trim()} onClick={()=>setStep(4)}>Configurer sa personnalité ↗</button></>}{step===4&&<><span className="builder-icon">✦</span><h3>Donnez-lui votre ton</h3><div className="tone-grid">{["Chaleureux","Expert","Énergique","Élégant"].map(item=><button className={tone===item?"active":""} onClick={()=>setTone(item)} key={item}>{item}</button>)}</div><div className="language-grid">{["Français","Lingala","Anglais","Kituba"].map(item=><label key={item}><input type="checkbox" checked={languages.includes(item)} onChange={()=>setLanguages(current=>current.includes(item)?current.filter(x=>x!==item):[...current,item])}/>{item}</label>)}</div><p>Réponses commerciales seulement, aucune prise de position personnelle.</p><button disabled={!languages.length} onClick={()=>setStep(5)}>Prévisualiser mon Jumeau numérique ↗</button></>}{step===5&&<><span className="builder-icon ready">✓</span><h3>Votre brouillon de Jumeau numérique est prêt</h3><p><strong>{tone}</strong>, en {languages.join(" et ")}, il présentera <strong>{product}</strong> à <strong>{price}</strong>. La génération IA finale nécessitera le service vidéo Wapi et votre validation avant publication.</p><div className="publish-options"><label><input type="checkbox" defaultChecked/> Boutique Wapi</label><label><input type="checkbox" defaultChecked/> Replay des directs</label><label><input type="checkbox"/> Réponses vidéo automatiques</label></div><button onClick={saveDraft}>Enregistrer en brouillon ✦</button></>}</div></section></div>;
}
void TwinSpace;
...
function ProfilePanel({ name,phone,accountId,syncStatus,photoUrl,founder,onSaveProfile,notify,onClose,go,onOpenShop,onOpenOrders,onSignOut }: { name:string;phone:string;accountId:string;syncStatus:string;photoUrl:string;founder:boolean;onSaveProfile:(name:string,photo:File|null)=>Promise<void>;notify:(message:string)=>void;onClose:()=>void;go:(space:Space)=>void;onOpenShop:()=>void;onOpenOrders:()=>void;onSignOut:()=>void }) {
  const [photoExpanded,setPhotoExpanded]=useState(false);
  const initials=name.split(/\s+/).map(part=>part[0]).join("").slice(0,2).toUpperCase();
return <div className="profile-layer"><button className="profile-dismiss" onClick={onClose} aria-label="Fermer le profil"/><aside className="profile-panel" role="dialog" aria-modal="true" aria-label="Mon profil"><header><button type="button" className={`profile-photo-button ${photoUrl?"has-photo":""}`} onClick={()=>photoUrl&&setPhotoExpanded(true)} disabled={!photoUrl} aria-label={photoUrl?"Agrandir ma photo de profil":"Aucune photo de profil"}><span style={photoUrl?{backgroundImage:`url(${photoUrl})`}:undefined}>{photoUrl?"":initials}</span>{photoUrl&&<small>AGRANDIR</small>}</button><div><small>{founder ? "COMPTE OFFICIEL WAPI" : "COMPTE WAPI"}</small><strong>{name} {founder && <i className="founder-grey-badge" title="Compte certifié">✓</i>}</strong>{founder && <em className="founder-title">Fondateur</em>}<p>{phone} · Vérifié</p></div><button onClick={onClose} aria-label="Fermer">×</button></header><div className="profile-account-card"><div><small>WAPI ID</small><strong>{accountId.slice(0,16)}</strong></div><div><small>TYPE DE COMPTE</small><strong>{founder?"Officiel · Fondateur":"Personnel"}</strong></div><div><small>SYNCHRONISATION</small><strong>{syncStatus==="synced"?"Cloud actif":syncStatus==="offline"?"Hors ligne":"Sécurisée"}</strong></div><p>Session protégée · paramètres de confidentialité et données sous votre contrôle</p></div><section><button onClick={()=>go("business")}><span>▥</span><div><strong>{founder ? "Dashboard Wapi by BCA" : "Business Suite"}</strong><small>{founder ? "Pilotage de Wapi by BCA et de l’équipe" : "Pages professionnelles et publicités"}</small></div><b>→</b></button><button onClick={onOpenShop}><span>◇</span><div><strong>Ma boutique</strong><small>Gérer mes annonces et mes ventes</small></div><b>→</b></button><button onClick={onOpenOrders}><span>▤</span><div><strong>Mes commandes</strong><small>Suivi, reçus et points de remise</small></div><b>→</b></button><button onClick={()=>go("twin")}><span>◎</span><div><strong>Mon Jumeau numérique</strong><small>Identité, voix, missions et autorisations</small></div><b>→</b></button><button onClick={()=>go("inbox")}><span>◫</span><div><strong>Mes conversations</strong><small>Messages, offres et commandes</small></div><b>→</b></button></section><ProfileEditor name={name} photoUrl={photoUrl} userKey={phone} onSave={onSaveProfile} notify={notify}/><div className="profile-safety"><span>✓</span><div><strong>{founder ? "Compte fondateur certifié" : "Identité protégée"}</strong><small>{founder ? "Badge gris officiel · Wapi by BCA" : "Un numéro unique pour votre compte"}</small></div></div><button className="profile-signout" onClick={onSignOut}>Se déconnecter</button></aside>{photoExpanded&&<div className="profile-photo-viewer" role="dialog" aria-modal="true" aria-label="Photo de profil agrandie"><button className="profile-photo-dismiss" onClick={()=>setPhotoExpanded(false)} aria-label="Fermer la photo"/><section><button onClick={()=>setPhotoExpanded(false)} aria-label="Fermer">×</button><div style={{backgroundImage:`url(${photoUrl})`}} role="img" aria-label={`Photo de profil de ${name}`}/><strong>{name}</strong><small>Photo synchronisée avec votre compte WAPI</small></section></div>}</div>;
}

function ContactProfilePanel({ contact, onClose, onMessage, onCall }: { contact: { name: string; phone?: string; mark: string; note: string; online: boolean }; onClose: () => void; onMessage: () => void; onCall: () => void }) {
  return <div className="profile-layer"><button className="profile-dismiss" onClick={onClose} aria-label="Fermer le profil"/><aside className="profile-panel contact-profile-panel" role="dialog" aria-modal="true" aria-label={`Profil de ${contact.name}`}>
    <header><span className="contact-profile-avatar">{contact.mark}<i className={contact.online ? "online" : ""}/></span><div><small>PROFIL WAPI</small><strong>{contact.name}</strong><p>{contact.online ? "En ligne maintenant" : "Contact Wapi"}</p></div><button onClick={onClose} aria-label="Fermer">×</button></header>
    <div className="contact-profile-status"><span>●</span><div><strong>{contact.online ? "Disponible" : "Hors ligne"}</strong><small>{contact.note}</small></div><em>{contact.online ? "ACTIF" : "RÉCENT"}</em></div>
    <div className="contact-profile-metrics"><article><strong>WAPI</strong><small>Compte vérifié</small></article><article><strong>PRIVÉ</strong><small>Conversation protégée</small></article><article><strong>24 H</strong><small>Stories temporaires</small></article></div>
    <section className="contact-profile-actions"><button onClick={onMessage}><span>◫</span><div><strong>Envoyer un message</strong><small>Ouvrir une conversation privée</small></div><b>→</b></button><button onClick={onCall}><span>☎</span><div><strong>Appeler</strong><small>Audio Wapi</small></div><b>→</b></button></section>
    <div className="contact-profile-info"><small>NUMÉRO WAPI</small><strong>{contact.phone || "Numéro masqué"}</strong><p>Les échanges restent protégés et vous pouvez contacter ce profil sans l’ajouter à vos contacts.</p></div>
  </aside></div>;
}
