...
export function TwinEngineStudio({ userId, userName, consent, setConsent, notify, cloud = true }: { userId: string; userName: string; consent: boolean; setConsent: (value: boolean) => void; notify: (text: string) => void; cloud?: boolean }) {
  ...
  useEffect(() => { if (!cloud) return; return watchTwinProfile(userId, setProfile, () => notifyRef.current("Profil du Jumeau numérique momentanément hors ligne")); }, [cloud, userId]);
  ...
  function speak() {
    ...
    notify(profile?.voiceUrl ? "Aperçu animé lancé — la voix système sera remplacée lors du rendu neuronal" : "Aperçu avec une voix système. Enregistrez votre voix pour préparer sa synthèse autorisée.");
  }
  async function persistConsent(value: boolean) {
    setConsent(value);
    if (!cloud) { updateLocalProfile({ identityConsent: value, voiceConsent: value, movementConsent: value }); notify(value ? "Mode aperçu du Jumeau numérique activé" : "Autorisations locales révoquées"); return; }
    try {
      ...
      notify(value ? "Consentement du Jumeau numérique enregistré" : "Autorisations du Jumeau numérique révoquées");
  ...
  async function createAutomation(data: { name: string; trigger: string; channel: string; action: string }) {
    if (!consent) { setTab("capture"); notify("Validez votre identité avant d’activer le Jumeau numérique"); return; }
    setBusy(true);
    try {
  ...
  async function addAutomation(event: FormEvent<HTMLFormElement>) {
    ...
    await createAutomation({ name: String(form.get("name") || "Mission du Jumeau numérique"), trigger: String(form.get("trigger") || "new-listing"), channel: String(form.get("channel") || "market"), action: String(form.get("action") || "prepare-video") });
  ...
  return <div className="twin-engine">
    <header className="twin-engine-hero">
      <div><span><i /> WAPI / JUMEAU NUMÉRIQUE</span><h2>Votre présence,<br /><em>multipliée.</em></h2><p>Un studio vivant pour chorégraphier votre Jumeau numérique, préparer votre voix et orchestrer vos interventions — toujours sous votre contrôle.</p><div><b>✓ Votre image uniquement</b><b>✓ Autorisation révocable</b><b>✓ Label IA permanent</b>{!cloud && <b className="local-mode">◎ APERÇU LOCAL IMMÉDIAT</b>}</div></div>
      <aside><div className="twin-ready-ring" style={{ "--ready": `${readiness * 20}%` } as CSSProperties}><strong>{readiness * 20}%</strong></div><div><small>STATUT DU JUMEAU</small><b>{readiness >= 4 ? "PRÊT À PRODUIRE" : "EN APPRENTISSAGE"}</b><span>{profile?.voiceUrl ? "Voix capturée" : "Voix à enregistrer"} · {profile?.movementUrl ? "Gestuelle capturée" : "Gestuelle à filmer"}</span></div></aside>
    </header>

    <section className="twin-governance" aria-label="Centre de contrôle du Jumeau numérique">
      <header><div><span>◆</span><div><small>CENTRE DE CONTRÔLE</small><h3>Votre identité reste sous votre autorité.</h3></div></div><button className={consent ? "authorized" : ""} disabled={busy} onClick={() => void persistConsent(!consent)}>{consent ? "Autorisé · Révoquer" : "Autoriser mon identité"}</button></header>
      <div className="twin-governance-grid">
        <article className={consent ? "ready" : ""}><span>{consent ? "✓" : "○"}</span><div><strong>Consentement vérifiable</strong><small>{consent ? "Autorisation active et révocable" : "Obligatoire avant toute capture"}</small></div></article>
        <article className={profile?.voiceUrl ? "ready" : ""}><span>{profile?.voiceUrl ? "✓" : "○"}</span><div><strong>Voix du propriétaire</strong><small>{profile?.voiceUrl ? "Empreinte enregistrée" : "Aucun modèle vocal actif"}</small></div></article>
        <article className={profile?.movementUrl ? "ready" : ""}><span>{profile?.movementUrl ? "✓" : "○"}</span><div><strong>Signature corporelle</strong><small>{profile?.movementUrl ? "Mouvements enregistrés" : "Capture à effectuer"}</small></div></article>
        <article className="ready"><span>✓</span><div><strong>Validation humaine</strong><small>Requise avant chaque publication</small></div></article>
      </div>
      <footer><span>IA signalée en permanence</span><span>Aucune imitation d’un tiers</span><span>Arrêt immédiat disponible</span></footer>
    </section>
    ...
    {tab === "engine" && <section className="motion-lab enhanced">
      <div className="motion-stage">
        <header className="stage-toolbar"><span><i /> APERÇU TEMPS RÉEL</span><div><button className={stageMode === "digital" ? "active" : ""} onClick={() => setStageMode("digital")}>Jumeau IA</button><button disabled={!signatureUrl} className={stageMode === "signature" ? "active" : ""} onClick={() => setStageMode("signature")}>Ma signature</button></div></header>
        {stageMode === "signature" && signatureUrl ? <video className="signature-preview" src={signatureUrl} autoPlay loop muted playsInline /> : <div className={`motion-avatar ${gesture} ${playing ? "playing" : ""}`} style={{ '--motion-intensity': intensity / 100, '--motion-tempo': `${1.45 - tempo / 100}s` } as CSSProperties}><div className="motion-aura" /><div className="avatar-head"><span>{initials}</span><i /><i /></div><div className="avatar-body"><span className="arm left" /><span className="torso"><b>IA</b></span><span className="arm right" /><span className="leg left" /><span className="leg right" /></div><small>JUMEAU NUMÉRIQUE DE {userName.toUpperCase()}</small></div>}
        <div className="motion-caption"><span>Créé avec mon Jumeau numérique IA</span><strong>{playing ? sequence[activeStep]?.label : "Prêt pour votre scénario"}</strong></div>
