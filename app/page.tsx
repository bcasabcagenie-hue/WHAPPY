...
import { cancelOrder, createGroup, createOrder, publishListing, publishRequest, removeListing, requestGroupJoin, sendConversationMessage, updateListing, watchConversationMessages, watchUserGroups, watchUserOrders, watchWhappyData, type CloudGroup, type CloudMessage, type CloudOrder } from "@/lib/whappy-data";
...
type Space = "orbit" | "live" | "market" | "barter" | "seek" | "inbox" | "calls" | "contacts" | "services" | "twin" | "business";
  ...
  return <main className="nova-shell whappy-blue">
    ...
    <section className="nova-stage">
      <header className="nova-topbar">
        ...
        <div className="top-actions">{demoMode&&<button className="demo-exit" onClick={()=>{setAuthenticated(false);setSpace("inbox");}}><span>×</span><small>Quitter la démo</small></button>}<span className={`sync-badge ${syncStatus}`} title={syncStatus==="synced"?"Données synchronisées":syncStatus==="syncing"?"Synchronisation en cours":syncStatus==="offline"?"Synchronisation indisponible":"Mode démonstration sans envoi de données"}><i/>{syncStatus==="synced"?"Cloud":syncStatus==="syncing"?"Sync…":syncStatus==="offline"?"Hors ligne":"Démo"}</span>{space === "inbox" ? <><button onClick={() => setOrdersOpen(true)}><span>▤</span><small>Commandes</small>{orders.length>0&&<b className="action-count">{orders.length}</b>}</button><button className="sell" onClick={() => { setDirectPhone(""); setDirectCompose((value)=>value+1); }}><span>＋</span><small>Nouveau</small></button></> : space === "calls" ? <><button onClick={() => go("contacts")}><span>◎</span><small>Contacts</small></button><button className="sell" onClick={() => { setDirectPhone(""); go("inbox"); setDirectCompose((value)=>value+1); }}><span>＋</span><small>Nouveau</small></button></> : <><button onClick={() => setOrdersOpen(true)}><span>▤</span><small>Commandes</small>{orders.length>0&&<b className="action-count">{orders.length}</b>}</button><button className="cart-action" onClick={()=>setCartOpen(true)}><span>◇</span><small>Panier</small>{cart.length>0&&<b>{cart.reduce((sum,line)=>sum+line.quantity,0)}</b>}</button><button className="sell" onClick={() => setModal("sell")}><span>＋</span><small>Vendre</small></button></>}</div>
      </header>
      ...
      {space === "inbox" && (userId ? <RealTimeInbox key={directCompose} embedded composeToken={directCompose} composePhone={directPhone} search={search} user={directUser} notify={notify} onCall={(peer,video)=>setCall({contact:peer.displayName,video,peer})}/> : <InboxSpace search={search} userId={userId} offer={demoOffer} setModal={setModal} notify={notify} onCall={(contact,video)=>setCall({contact,video})} />)}
      ...
      {space === "business" && <BusinessStudio userId={userId||"demo-user"} userName={auth.currentUser?.displayName||profileName||"Cyril Bokilo"} search={search} notify={notify} demo={demoMode}/>}
    </section>
...
function InboxSpace({ search, userId, offer, setModal, notify, onCall }: { search:string; userId:string; offer:{id:number;text:string}|null; setModal:(type:"message")=>void; notify:(text:string)=>void; onCall:(contact:string,video:boolean)=>void }) {
  ...
  useEffect(()=>{if(!offer||handledOffer.current===offer.id)return;handledOffer.current=offer.id;setSent((current)=>({...current,[selected]:[...(current[selected]||[]),{id:`offer-${offer.id}`,text:offer.text,senderId:"local"}]}));},[offer,selected]);
...
function ListingCard({ item, saved, onSave, onOpen }: { item:Listing; saved:boolean; onSave:()=>void; onOpen:()=>void }) {
  return <article className="listing-card"><button className={`save ${saved?"active":""}`} onClick={onSave}>{saved?"♥":"♡"}</button><button className={`listing-art ${item.tone} ${item.mediaUrl?"has-media":""}`} style={item.mediaUrl?{backgroundImage:`linear-gradient(180deg,transparent 45%,rgba(2,13,18,.72)),url(${item.mediaUrl})`}:undefined} onClick={onOpen}>{!item.mediaUrl&&<span>{item.mark}</span>}<small>{item.category}</small>{item.mode==="troc"&&<b>⇄ TROC</b>}{item.status==="reserved"&&<em className="reserved-badge">RÉSERVÉ</em>}</button><div><span className="seller"><i>{item.mark}</i>{item.seller}<b>✓</b><small>{item.trust}% fiable</small></span><h3>{item.title}</h3><strong>{item.price}</strong><p>⌖ {item.place}</p><button onClick={onOpen}>{item.mode==="troc"?"Proposer un échange":"Discuter"} ↗</button></div></article>;
}
