...
function InboxSpace({ search, setModal, notify }: { search:string; setModal:(type:"message")=>void; notify:(text:string)=>void }) {
  const [selected,setSelected]=useState(0); const [text,setText]=useState(""); const [mobileChat,setMobileChat]=useState(false);
  ...
  return <div className={`inbox-space ${mobileChat?"chat-open":""}`}><aside className="inbox-list"><div className="inbox-title"><div><small>MESSAGERIE PRIORITAIRE</small><strong>Discussions <span>3 nouvelles</span></strong></div><button onClick={()=>notify("Nouvelle conversation prête")} aria-label="Nouvelle conversation">＋</button></div><div className="inbox-presence"><button><span className="presence-me">CB<i>＋</i></span><small>Mon statut</small></button>{messages.slice(0,3).map(m=><button key={m.name} onClick={()=>notify(`Statut de ${m.name} ouvert`)}><span>{m.mark}<i/></span><small>{m.name.split(" ")[0]}</small></button>)}</div><div className="inbox-filters"><button className="active">Tout</button><button>Non lus</button><button>Groupes</button><button>Affaires</button></div>{visibleMessages.map(({message:m,index})=><button className={selected===index?"active":""} onClick={()=>{setSelected(index);setMobileChat(true)}} key={m.name}><span className="profile-avatar"><Mark color={m.color}>{m.mark}</Mark><i/></span><span><strong>{m.name}<em>{m.badge}</em></strong><small>{m.text}</small></span><i>{m.time}</i>{m.unread>0&&<b>{m.unread}</b>}</button>)}{visibleMessages.length===0&&<p className="empty-messages">Aucune conversation trouvée.</p>}</aside><section className="deal-chat"><header><button className="chat-back" onClick={()=>setMobileChat(false)} aria-label="Retour aux discussions">←</button><span className="profile-avatar large"><Mark color={messages[selected].color}>{messages[selected].mark}</Mark><i/></span><div><strong>{messages[selected].name}</strong><small><i/> En ligne · Identité vérifiée</small></div><button onClick={()=>notify("Appel audio démarré")} aria-label="Appel audio">☎</button><button onClick={()=>notify("Appel vidéo démarré")} aria-label="Appel vidéo">▣</button><button aria-label="Options">•••</button></header><div className="profile-pulse"><span>✦</span><div><small>{messages[selected].badge} · {messages[selected].streak} ÉCHANGES RÉUSSIS</small><strong>{messages[selected].mood}</strong></div><button onClick={()=>notify(`Profil de ${messages[selected].name} ouvert`)}>Voir le profil ↗</button></div><div className="deal-context"><span className="product-thumb">◇</span><div><small>CONVERSATION LIÉE À UNE OPPORTUNITÉ</small><strong>{selected===0?"Canapé modulable en velours":"MacBook Air M3 · Comme neuf"}</strong><p>{selected===0?"Échange accepté":"750 000 FCFA"}</p></div><button onClick={()=>notify("Annonce ouverte")}>Voir l&apos;annonce ↗</button></div><div className="deal-messages" key={selected}><span className="chat-date">AUJOURD&apos;HUI</span><div className="theirs">Bonjour ! Est-ce que votre annonce est toujours disponible ?<small>12:03</small></div><div className="mine">Oui, absolument. On peut aussi discuter d&apos;un échange.<small>12:05 <b>✓✓</b></small></div><div className="theirs">Parfait, je vous envoie ma proposition.<small>12:08</small></div><span className="typing"><i/><i/><i/> {messages[selected].name.split(" ")[0]} écrit…</span></div><form onSubmit={send}><button type="button" aria-label="Ajouter une pièce jointe">＋</button><button type="button" aria-label="Ajouter un emoji">☺</button><input value={text} onChange={e=>setText(e.target.value)} placeholder="Écrire un message…"/><button className="voice-action" type="button" onClick={()=>notify("Maintenez pour enregistrer un message vocal")} aria-label="Message vocal">◉</button><button className="offer-action" type="button" onClick={()=>setModal("message")}>◇ Offre</button><button className="send-action" type="submit" aria-label="Envoyer">➤</button></form></section></div>;
}
